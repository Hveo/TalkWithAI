...
namespace InfimaGames.LowPolyShooterPack
{
	...
	public sealed class Character : CharacterBehaviour
	{
		...
		public void OnTryTalk(InputAction.CallbackContext context)
        {
			if (!VOSKListener)
				return;
            ...
            {
				case { phase: InputActionPhase.Started }:
					VOSKListener.StartListening();
					audioService?.PlayOneShot(TalkieIn, settings);
					break;
                
				case { phase: InputActionPhase.Canceled }:
					VOSKListener.RequestStopListening();
					audioService?.PlayOneShot(TalkieOut, settings);
					break;	
